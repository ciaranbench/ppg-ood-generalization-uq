...
class Main_PPG(lp.LightningModule):
    ...
    # def _step(self,data_batch, batch_idx, train, test=False, dataloader_idx=0):
        ...
        # # Load the weights tensor and bins array from the specified files
        # weights_tensor = torch.tensor(np.load(self.hparams.weights_file))
        # bins = np.load(self.hparams.bins_file)
        
        # ## 200 bins



        # # Extract SBP values and find the corresponding weight bins
        # sbp_values = data_batch[1][:, 0]  # Assuming SBP values are in the first column of labels
        

        # # Calculate weights for each batch element
        # bin_indices = np.digitize(sbp_values.cpu().numpy(), bins) - 1
        # bin_indices = np.clip(bin_indices, 0, len(weights_tensor) - 1)
        # sample_weights = weights_tensor[bin_indices]
        
        # #Ensure sample_weights has the right shape for broadcasting
        # sample_weights = sample_weights.to(data_batch[0].device)  # Move to correct device
        # sample_weights = sample_weights.view(-1, 1)  # Reshape to [batch_size, 1]
        ...
        # else:
            ...
            # loss_unweighted = self.criterion(preds_all, data_batch[1], reduction='none')
            # loss_unweighted = loss_unweighted.mean(dim=1, keepdim=True)  # Ensure loss_unweighted has shape [batch_size, 1]
            # loss = (loss_unweighted * sample_weights).mean()
    ...
    def _step(self, data_batch, batch_idx, train, test=False, dataloader_idx=0):
        ...
        # Print the first 5 rows of SBP and DBP values to verify the assumption
        #print(data_batch[1][:5])  # Display the first 5 rows of the label data (SBP and DBP)

        # Load SBP and DBP weights and fixed bins
        sbp_weights_tensor = torch.tensor(np.load(self.hparams.sbp_weights_file))  # Load SBP weights
        dbp_weights_tensor = torch.tensor(np.load(self.hparams.dbp_weights_file))  # Load DBP weights
        fixed_bins = np.load(self.hparams.bins_file)  # Load fixed bins (assumed to be shared for SBP and DBP)

        # Extract SBP and DBP values from the labels (assuming SBP is column 0 and DBP is column 1)
        sbp_values = data_batch[1][:, 0]  # SBP
        dbp_values = data_batch[1][:, 1]  # DBP

        # Calculate SBP weights
        sbp_bin_indices = np.digitize(sbp_values.cpu().numpy(), fixed_bins) - 1
        sbp_bin_indices = np.clip(sbp_bin_indices, 0, len(sbp_weights_tensor) - 1)  # Ensure indices are within range
        sbp_sample_weights = sbp_weights_tensor[sbp_bin_indices]

        # Calculate DBP weights
        dbp_bin_indices = np.digitize(dbp_values.cpu().numpy(), fixed_bins) - 1
        dbp_bin_indices = np.clip(dbp_bin_indices, 0, len(dbp_weights_tensor) - 1)  # Ensure indices are within range
        dbp_sample_weights = dbp_weights_tensor[dbp_bin_indices]

        # Move weights to the same device as the input data
        sbp_sample_weights = sbp_sample_weights.to(data_batch[0].device)
        dbp_sample_weights = dbp_sample_weights.to(data_batch[0].device)

        # Ensure sample_weights have the right shape for broadcasting
        sbp_sample_weights = sbp_sample_weights.view(-1, 1)  # Shape [batch_size, 1]
        dbp_sample_weights = dbp_sample_weights.view(-1, 1)  # Shape [batch_size, 1]

        # Loss calculation
        if self.hparams.finetune_dataset == "mimiciiibp":
            # For "mimiciiibp", we calculate a simple unweighted loss
            loss = self.criterion(preds_all.view(-1), data_batch[1].view(-1))  # Flatten all predictions and labels
        else:
            # Calculate unweighted loss for SBP and DBP separately
            loss_unweighted_sbp = self.criterion(preds_all[:, 0], data_batch[1][:, 0], reduction='none')  # SBP
            loss_unweighted_dbp = self.criterion(preds_all[:, 1], data_batch[1][:, 1], reduction='none')  # DBP

            # Reshape to ensure losses have the correct shape
            loss_unweighted_sbp = loss_unweighted_sbp.view(-1, 1)
            loss_unweighted_dbp = loss_unweighted_dbp.view(-1, 1)

            # Apply the sample weights
            loss_weighted_sbp = (loss_unweighted_sbp * sbp_sample_weights).mean()
            loss_weighted_dbp = (loss_unweighted_dbp * dbp_sample_weights).mean()

            # Combine the SBP and DBP losses (you can adjust the weighting between SBP and DBP if needed)
            loss = (loss_weighted_sbp + loss_weighted_dbp) / 2

        # Log the loss for the appropriate phase (train, test, validation)
        self.log("train_loss" if train else ("test_loss" if test else "val_loss"), loss)

        # Storing predictions and targets for evaluation
        if not train and not test:
            self.val_preds[dataloader_idx].append(preds_all.detach())
            self.val_targs[dataloader_idx].append(data_batch[1])
        elif not train and test:
...
def add_application_specific_args(parser):
    ...
    #parser.add_argument('--weights-file', type=str, required=True, help='Path to the weights tensor file.')
    #parser.add_argument('--bins-file', type=str, required=True, help='Path to the bins file.')
    
    parser.add_argument('--sbp-weights-file', type=str, required=True, help='Path to the weights tensor file.')
    parser.add_argument('--bins-file', type=str, required=True, help='Path to the bins file.')
    parser.add_argument('--dbp-weights-file', type=str, required=True, help='Path to the weights tensor file.')
